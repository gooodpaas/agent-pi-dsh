// Create an immutable draft v3.6.7 release, then publish it only after all
...
const sourceAsset = 'Agent-Pi-DSH-3.6.2-CAD-corresponding-source.tar.gz'
const sourceChecksumAsset = 'Agent-Pi-DSH-3.6.2-CAD-corresponding-source.tar.gz.sha256'
const windowsBuildReceiptAsset = 'Agent-Pi-DSH-3.6.7-x64.exe.build.json'
const platformChecksumPairs = [
  ['Agent-Pi-DSH-3.6.7-mac-arm64.dmg', 'Agent-Pi-DSH-3.6.7-mac-arm64.dmg.sha256'],
  ['Agent-Pi-DSH-3.6.7-mac-arm64.zip', 'Agent-Pi-DSH-3.6.7-mac-arm64.zip.sha256'],
  ['Agent-Pi-DSH-3.6.7-linux-x86_64.AppImage', 'Agent-Pi-DSH-3.6.7-linux-x86_64.AppImage.sha256'],
  ['Agent-Pi-DSH-3.6.7-linux-amd64.deb', 'Agent-Pi-DSH-3.6.7-linux-amd64.deb.sha256'],
]
...
function assertReleaseCheckout() {
  ...
  if (status) throw new Error('v3.6.7 publishing requires a completely clean checkout')
...
async function assertCadDistributionReady() {
  ...
  await assertChecksumPair('Agent-Pi-DSH-3.6.7-x64.exe')
  await assertChecksumPair('runtime-payload-3.6.7.tar.gz')
  ...
  verifyWindowsBuildReceipt({
    root,
    installerPath: join(root, 'release', 'Agent-Pi-DSH-3.6.7-x64.exe'),
  ...
  assertUniverPublicReleaseArchive(join(root, 'release', 'runtime-payload-3.6.7.tar.gz'))
}

if (!['--create-draft', '--publish'].includes(mode)) {
  throw new Error('Usage: publish-v3.6.7-release.mjs --create-draft | --publish')
}
...
const body = readFileSync(join(root, 'release/github-notes-3.6.7.md'), 'utf8')
const requiredAssets = new Set([
  'Agent-Pi-DSH-3.6.7-x64.exe',
  'Agent-Pi-DSH-3.6.7-x64.exe.sha256',
  windowsBuildReceiptAsset,
  'Agent-Pi-DSH-3.6.7-mac-arm64.dmg',
  'Agent-Pi-DSH-3.6.7-mac-arm64.dmg.sha256',
  'Agent-Pi-DSH-3.6.7-mac-arm64.zip',
  'Agent-Pi-DSH-3.6.7-mac-arm64.zip.sha256',
  'Agent-Pi-DSH-3.6.7-linux-x86_64.AppImage',
  'Agent-Pi-DSH-3.6.7-linux-x86_64.AppImage.sha256',
  'Agent-Pi-DSH-3.6.7-linux-amd64.deb',
  'Agent-Pi-DSH-3.6.7-linux-amd64.deb.sha256',
  'runtime-payload-3.6.7.tar.gz',
  'runtime-payload-3.6.7.tar.gz.sha256',
  sourceAsset,
  sourceChecksumAsset,
])
