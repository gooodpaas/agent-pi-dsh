...
async def _render(html, pdf, title):
    ...
    async with async_playwright() as pw:
        ...
        pg = await b.new_page()
        await pg.goto(Path(html).resolve().as_uri(), wait_until="networkidle")
...
def page_count(pdf):
    out = subprocess.run(["pdfinfo", pdf], capture_output=True, text=True, encoding="utf-8", check=True).stdout
    return int(out.split("Pages:")[1].split()[0])


def page_texts(pdf, n):
    return {p: subprocess.run(["pdftotext", "-enc", "UTF-8", "-f", str(p), "-l", str(p), pdf, "-"],
...
def patch_toc(builder, found):
    ...
    """
    t = Path(builder).read_text(encoding="utf-8")
    orig = t
    for key, no in found.items():
        t = re.sub(rf'(toc_row\(\s*"{re.escape(key)}"\s*,\s*"[^"]*"\s*,\s*)\d+',
                   rf'\g<1>{no}', t)
    if t != orig:
        Path(builder).write_text(t, encoding="utf-8")
    return t != orig
